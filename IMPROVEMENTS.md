# MoFrame — План улучшений

## 🐞 Известные баги (Tech Debt)

### Тесты: 2 из 21 падают

| Тест | Проблема | Приоритет |
|------|----------|-----------|
| `test_zoom` | Zoom < 1.0 увеличивает вместо уменьшения. `_apply_zoom` неправильно считает new_w/new_h когда zoom < 1 | Низкий |
| `test_detect_single_panel` | Детектор находит 4 панели вместо 1. Пороги слишком чувствительные для искусственных изображений | Низкий |

**Записано:** 2026-04-23

---

## 🔴 Критичные (P0)

### 1. Улучшить React ↔ Streamlit синхронизацию
**Статус:** ✅ **ВЫПОЛНЕНО** — Вариант B (HTTP) работает!

**Решение:** HTTP сервер на порту 8765
- React отправляет JSON при каждом изменении
- Streamlit получает из глобальной очереди (builtins)
- Авто-синхронизация без copy/paste
- Кнопка "🔄 Sync from Editor" для ручного обновления

**Выполнено:** 2026-04-23

---

### 2. Добавить preview перед рендером ⭐ **В РАБОТЕ**
**Проблема:** Пользователь не видит результат до полного рендера

**Решение:**
- Генерировать 5-10 кадров превью для первого перехода
- Показывать как GIF или слайдер
- Кнопка "Preview" перед "Render"

**Оценка:** 2-3 часа
**Приоритет:** Высокий

---

## 🟡 Важные (P1)

### 3. Улучшить морфинг
**Текущее:** Ken Burns, Crossfade, Slide, Zoom

**Добавить:**
- **AI Interpolation** — Stable Diffusion для промежуточных кадров
- **Feature-based warp** — OpenCV optical flow
- **Smart zoom** — авто-определение фокусной точки

**Оценка:** 8-16 часов
**Приоритет:** Средний (требует исследований)

---

### 4. Аудио дорожка
**Статус:** ✅ **ВЫПОЛНЕНО** — Добавлена в Step 4 (Render)

**Функционал:**
- Загрузка аудио (MP3, WAV, OGG, M4A, AAC)
- Регулятор громкости (0-2x)
- Fade in/out (0-2 сек)
- **Auto-fit** — авто-подгонка длительности панелей под длину аудио
- Показ suggested panel duration
- Аудио миксируется через moviepy в renderer

**Выполнено:** 2026-04-24

---

### 5. Пресеты для соцсетей
**Статус:** ✅ **ВЫПОЛНЕНО** — Добавлены 5 пресетов в Step 4 (Render)

**Пресеты:**
- 🎨 **Custom** — ручные настройки
- 📱 **Instagram Reels** (9:16, 30 FPS, 60 sec max)
- 🎵 **TikTok** (9:16, 30 FPS, 180 sec max)
- 📺 **YouTube Shorts** (9:16, 30 FPS, 60 sec max)
- ▶️ **YouTube Landscape** (16:9, 24 FPS)
- 🐦 **Twitter/X** (1:1, 24 FPS)

**Функционал:**
- Селект пресета в sidebar Step 4
- Авто-установка resolution, FPS, max duration
- Ограничение panel duration если preset имеет max_duration
- Авто-суффикс в имени файла (например `my_comic_instagram_reels.mp4`)
- Показ estimated video duration
- Поддерживаемые разрешения: 16:9 (Full HD, HD, SD), 9:16 (Vertical, Vertical HD), 1:1 (Square)

**Выполнено:** 2026-04-24

---

## 🟢 Желательные (P2)

### 6. Batch processing
**Функционал:**
- Загрузка нескольких комиксов
- Очередь на рендер
- Прогресс по всем файлам
- Zip с результатами

**Оценка:** 4-6 часов
**Приоритет:** Низкий

---

### 7. Улучшенная детекция панелей
**Текущее:** OpenCV contours

**Улучшения:**
- ML модель (YOLO) для сложных случаев
- Детекция speech bubbles
- Авто-пропуск пустых страниц

**Оценка:** 8-12 часов
**Приоритет:** Низкий (требует датасет)

---

### 8. Экспорт в разные форматы
**Добавить:**
- WebM (для web)
- GIF (для соцсетей)
- APNG (анимированный PNG)

**Оценка:** 2-3 часа
**Приоритет:** Низкий

---

## 📊 Roadmap по времени

| Неделя | Задачи |
|--------|--------|
| **1** | ✅ P0-1 (HTTP sync), ✅ P0-2 (preview) |
| **2** | ✅ P1-5 (пресеты), 🔄 P1-4 (аудио) |
| **3** | P1-3 (морфинг research) |
| **4** | P2-6 (batch), P2-8 (экспорт), Tech Debt (баги) |

---

## 🎯 Текущий статус

**P0-1 Вариант A:** Streamlit Custom Component — ❌ **ОТКАТ** (не работает)

Рабочий вариант: iframe + JSON copy/paste

**Попытка Custom Component:**
- Создана структура в `ui/components/panel_editor_component/`
- Настроен webpack с html-webpack-plugin
- Компонент не загружается в Streamlit (404, CORS, iframe sandbox)
- **Вывод:** Streamlit Custom Component с React — слишком сложно

**Следующий шаг:** P0-1 Вариант B (WebSocket) или P0-2 (Preview)

---

Последнее обновление: 2026-04-23
